...
class EFolder2Controller extends Controller
{
    public function index()
    {
        $documents = Document::orderBy('doc_name')->get();
        return view('electrical.layer2.efolder2', compact('documents'));
    }
    public function upload(Request $request)
    {
        activity()->withoutLogs(function () use ($request) {
            if ($request->hasFile('file')) {
                ...
                $path = $file->store('electrical/EFolder2', 'public');

                // Simpan data dokumen ke dalam database
                $document = new Document;
                $document->doc_name = $file_name;
                $document->category = "EFolder2";
                $document->path = $path;
    ...
    public function delete($id)
    {
        ...
        return redirect()->route('EFolder2')->with('success', 'Dokumen Berhasil Dihapus!');
    }
    public function destroy($category)
    {
        ...
        return redirect()->route('EFolder2')->with('success', 'Dokumen EFolder2 Berhasil Dihapus');
    }
    public function password(Request $request, $id)
    {
        ...
        if ($password === $passwordDoc->password) {
            return redirect()->route('open-EFolder2', $document->id);
        } else {
