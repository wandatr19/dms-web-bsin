...
class EInAPPD1Controller extends Controller
{
    public function index()
    {
        $documents = Document::orderBy('doc_name')->get();
        return view('electrical.layer2.einappd1', compact('documents'));
    }
    public function upload(Request $request)
    {
        activity()->withoutLogs(function () use ($request) {
            if ($request->hasFile('file')) {
                ...
                $path = $file->store('electrical/EInAPPD1', 'public');

                // Simpan data dokumen ke dalam database
                $document = new Document;
                $document->doc_name = $file_name;
                $document->category = "EInnerAPPD1";
                $document->path = $path;
    ...
    public function delete($id)
    {
        ...
        return redirect()->route('EInAPPD1')->with('success', 'Dokumen Berhasil Dihapus!');
    }
    public function destroy($category)
    {
        ...
        return redirect()->route('EInAPPD1')->with('success', 'Dokumen EInnerAPPD1 Berhasil Dihapus');
    }
    public function password(Request $request, $id)
    {
        ...
        if ($password === $passwordDoc->password) {
            return redirect()->route('open-EInAPPD1', $document->id);
        } else {
