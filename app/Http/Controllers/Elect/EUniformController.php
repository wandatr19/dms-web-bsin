...
class EUniformController extends Controller
{
    ...
    public function upload(Request $request)
    {
        activity()->withoutLogs(function () use ($request) {
            if ($request->hasFile('file')) {
                ...
                $path = $file->store('electrical/EUniform', 'public');

                // Simpan data dokumen ke dalam database
                $document = new Document;
                $document->doc_name = $file_name;
                $document->category = "EUniformity";
                $document->path = $path;
    ...
    public function delete($id)
    {
        ...
        return redirect()->route('EUniform')->with('success', 'Dokumen Berhasil Dihapus!');
    }
    public function destroy($category)
    {
        ...
        return redirect()->route('EUniform')->with('success', 'Dokumen EUniformity Berhasil Dihapus');
    }
    public function password(Request $request, $id)
    {
        ...
        if ($password === $passwordDoc->password) {
            return redirect()->route('open-EUniform', $document->id);
        } else {
