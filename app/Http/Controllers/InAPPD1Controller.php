...
class InAPPD1Controller extends Controller
{
    public function index()
    {
        $documents = Document::orderBy('doc_name')->get();
        return view('mechanical.layer2.InAPPD1', compact('documents'));
    }
    public function upload(Request $request)
    {
        activity()->withoutLogs(function () use ($request) {
            if ($request->hasFile('file')) {
                ...
                $path = $file->store('mechanical/InAPPD1', 'public');

                // Simpan data dokumen ke dalam database
                $document = new Document;
                $document->doc_name = $file_name;
                $document->category = "InnerAPPD1";
                $document->path = $path;
    ...
    public function delete($id)
    {
        ...
        return redirect()->route('InAPPD1')->with('success', 'Dokumen Berhasil Dihapus!');
    }
    public function destroy($category)
    {
        ...
        return redirect()->route('InAPPD1')->with('success', 'Dokumen InAPPD1 Berhasil Dihapus');
    }
    public function password(Request $request, $id)
    {
        ...
        if ($password === $passwordDoc->password) {
            return redirect()->route('open-InAPPD1', $document->id);
        } else {
