...
class MonowireController extends Controller
{
    public function index()
    {
        $documents = Document::orderBy('doc_name')->get();
        return view('mechanical.layer2.monowire', compact('documents'));
    }
    public function upload(Request $request)
    {
        activity()->withoutLogs(function () use ($request) {
            if ($request->hasFile('file')) {
                ...
                $path = $file->store('mechanical/monowire', 'public');

                // Simpan data dokumen ke dalam database
                $document = new Document;
                $document->doc_name = $file_name;
                $document->category = "monowire";
                $document->path = $path;
    ...
    public function delete($id)
    {
        ...
        return redirect()->route('monowire')->with('success', 'Dokumen Berhasil Dihapus!');
    }
    public function destroy($category)
    {
        ...
        return redirect()->route('monowire')->with('success', 'Dokumen monowire Berhasil Dihapus');
    }
    public function password(Request $request, $id)
    {
        ...
        if ($password === $passwordDoc->password) {
            return redirect()->route('open-monowire', $document->id);
        } else {
