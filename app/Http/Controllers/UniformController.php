...
class UniformController extends Controller
{
    public function index()
    {
        ...
        return view('mechanical.layer2.uniformity', compact('documents'));
    }
    public function upload(Request $request)
    {
        activity()->withoutLogs(function () use ($request) {
            if ($request->hasFile('file')) {
                ...
                $path = $file->store('mechanical/Uniform', 'public');

                // Simpan data dokumen ke dalam database
                $document = new Document;
                $document->doc_name = $file_name;
                $document->category = "Uniformity";
                $document->path = $path;
    ...
    public function delete($id)
    {
        ...
        return redirect()->route('Uniform')->with('success', 'Dokumen Berhasil Dihapus!');
    }
    public function destroy($category)
    {
        ...
        return redirect()->route('Uniform')->with('success', 'Dokumen Uniform Berhasil Dihapus');
    }
    public function password(Request $request, $id)
    {
        ...
        if ($password === $passwordDoc->password) {
            return redirect()->route('open-Uniform', $document->id);
        } else {
