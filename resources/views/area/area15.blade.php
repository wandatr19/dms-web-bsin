    ...
    <div class="modal fade" id="exampleModalDelete" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content shadow" style="border-radius: 15px">
          ...
          <a href="{{route('destroy-Area15', ['category'=>'Area15'])}}">
  ...
  <tbody>
    @foreach ($documents as $document)
      @if ($document->category == "Area15")
          <tr>
              <td><a href="{{ route('view-Area15', $document->id) }}" target="_blank">{{ $document->doc_name }}</a></td>
                ...
                <div class="modal fade" id="exampleModalOpen{{ $document->id }}" tabindex="-1" aria-labelledby="exampleModalOpen{{ $document->id }}" aria-hidden="true">
                  <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content shadow px-5" style="border-radius: 15px">
                      ...
                      <!-- buat input password -->
                      <div class="mb-3 row">
                        <form action="{{route('pw-Area15', $document->id)}}" method="POST" target="_blank">
                          @csrf
              ...
              @if (auth()->user()->role == "admin")
                ...
                <td style="text-align: center"><a href="{{ route('del-Area15', ['id' => $document->id]) }}" type="button"
  ...
  <div class="modal fade" id="exampleModalbtnplus" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg">
      <div class="modal-content shadow p-3" style="border-radius: 15px">
        ...
        <div class="text-center">
        <!-- Dropzone Baru -->
          <form action="{{route('up-Area15')}}" method="POST" encytype="multipart/form-data" id="pdf-upload" class="dropzone">
            @csrf
          </form>  
        <!-- Akhri Dropzone baru -->
