    ...
    <div class="modal fade" id="exampleModalDelete" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content shadow" style="border-radius: 15px">
          ...
          <a href="{{route('destroy-TR', ['category'=>'TireRepair'])}}">
  ...
  <tbody>
    @foreach ($documents as $document)
      @if ($document->category == "TireRepair")
          <tr>
              <td><a href="{{ route('view-TR', $document->id) }}" target="_blank">{{ $document->doc_name }}</a></td>
                ...
                <div class="modal fade" id="exampleModalOpen{{ $document->id }}" tabindex="-1" aria-labelledby="exampleModalOpen{{ $document->id }}" aria-hidden="true">
                  <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content shadow px-5" style="border-radius: 15px">
                      ...
                      <!-- buat input password -->
                      <div class="mb-3 row">
                        <form action="{{route('pw-TR', $document->id)}}" method="POST" target="_blank">
                          @csrf
              ...
              @if (auth()->user()->role == "admin")
                ...
                <td style="text-align: center"><a href="{{ route('del-TR', ['id' => $document->id]) }}" type="button"
  ...
  <div class="modal fade" id="exampleModalbtnplus" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg">
      <div class="modal-content shadow p-3" style="border-radius: 15px">
        ...
        <div class="text-center">
        <!-- Dropzone Baru -->
          <form action="{{route('up-TR')}}" method="POST" encytype="multipart/form-data" id="pdf-upload" class="dropzone">
            @csrf
          </form>  
        <!-- Akhri Dropzone baru -->
